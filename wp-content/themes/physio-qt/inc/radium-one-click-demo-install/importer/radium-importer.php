	...
	class Radium_Theme_Importer {
	    ...
	    public function demo_installer() {
			...
			if( !empty($this->imported_demos ) ) {
				...
				$button_text = esc_html__('Import Again', 'physio-qt');

			} else {

				$button_text = esc_html__('Import Demo Data', 'physio-qt');
				...
				<form method="post">
					...
					<?php if( !empty($this->imported_demos) ) { ?>
						...
						<div style="background-color: #FAFFFB; margin:10px 0;padding: 5px 10px;color: #8AB38A;border: 2px solid #a1d3a2; clear:both; width:90%; line-height:18px;">
							<p><?php esc_html_e( 'Demo already imported', 'physio-qt' ); ?></p>
						</div>
	    ...
	    public function set_demo_theme_options( $file ) {
			...
			if ( file_exists( $file ) ) {
				...
				if ( !empty( $data ) || is_array( $data ) ) {
	      		...
	      		wp_die(
      				esc_html__( 'Theme options Import file could not be found. Please try again.', 'physio-qt' ),
      				'',
	    ...
	    function process_widget_import_file( $file ) {
	    	...
	    	if ( ! file_exists( $file ) ) {
	    		wp_die(
	    			esc_html__( 'Widget Import file could not be found. Please try again.', 'physio-qt' ),
	    			'',
	    			array( 'back_link' => true )
	    		);
	    	}
	    ...
	    public function import_widgets( $data ) {
	    	...
	    	foreach ( $data as $sidebar_id => $widgets ) {
	    		...
	    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
	    			...
	    			$sidebar_message = esc_html__( 'Sidebar does not exist in theme (using Inactive)', 'physio-qt' );
	    		}
	    		...
	    		foreach ( $widgets as $widget_instance_id => $widget ) {
	    			...
	    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
	    				...
	    				$widget_message = esc_html__( 'Site does not support widget', 'physio-qt' ); // explain why widget not imported
	    			}
	    			...
	    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
	    				...
	    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
	    					...
	    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
	    						...
	    						$widget_message = esc_html__( 'Widget already exists', 'physio-qt' ); // explain why widget not imported
	    			...
	    			if ( ! $fail ) {
	    				...
	    				if ( $sidebar_available ) {
	    					...
	    					$widget_message = esc_html__( 'Imported', 'physio-qt' );
	    				} else {
	    					$widget_message_type = 'warning';
	    					$widget_message = esc_html__( 'Imported to Inactive', 'physio-qt' );
	    				}
	    			...
	    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_html__( 'No Title', 'physio-qt' ); // show "No Title" if widget instance is untitled
