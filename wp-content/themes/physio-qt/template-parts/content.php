...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'clearfix' ); ?>>
		...
		<div class="hentry--post-thumbnail">
				<a href="<?php esc_url( the_permalink() ); ?>" class="hentry--thumbnail">
					...
					<?php if ( 'yes' === get_theme_mod( 'blog_date_label', 'yes' ) ) : ?>
						<time datetime="<?php esc_attr( the_time( 'c' ) ); ?>" class="meta-data--date"><?php echo esc_attr( get_the_date() ); ?></time>
					<?php endif; ?>
				</a>
		</div>
	<?php endif; ?>
	...
	<div class="hentry--content">
		...
		<?php the_title( sprintf( '<h2 class="hentry--title"><a href="%s">', esc_url( get_permalink() ) ), '</a></h2>' ); ?>

		<?php
			the_content( sprintf(
				/* translators: %s: Name of current post. */
				wp_kses( __( 'Continue reading %s', 'physio-qt' ), array( 'span' => array( 'class' => array() ) ) ),
